...
map<pair<ll,ll>,ll> my;

ll m,n;
ll f(ll pos,ll last,ll *a){
    if(pos==n){
        // Base Case
        return 1;
    }
    
    if(my.count(make_pair(pos,last))){
        return my[make_pair(pos,last)];
    }
    ...
    for(ll i=0;i<m;i++){
        if(abs(a[i]-last)<=2 || last==-1){
            ans+=f(pos+1,a[i],a);
        }
    }
    return my[make_pair(pos,last)]=ans;
