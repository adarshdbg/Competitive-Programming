...
int main() {
    ...
    ll pre[b.size()],suf[b.size()];
    for(ll i=0;i<b.size();i++){
        pre[i]=(i)?pre[i-1]+m[b[i]]:m[b[i]];
    }
    for(ll i=b.size()-1;i>=0;i--){
        suf[i]=(i!=b.size()-1)?suf[i+1]+m[b[i]]:m[b[i]];
    }
    p=power(2,min((ll)20,(ll)(p*8.0/n)));
    ll ans=1e9;
    // cout<<p<<endl;
    for(ll i=0;i<b.size();i++){
        // cout<<b[i]<<" ";
        ll ind=i+p-1;
        if(ind!=-1){
            ans=min(ans,(i?pre[i-1]:0)+(ind<b.size()-1?suf[ind+1]:0));
        }
    }
    cout<<ans<<endl;
